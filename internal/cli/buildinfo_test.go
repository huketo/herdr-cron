...
func TestResolveFrom(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// go install github.com/huketo/herdr-cron/cmd/herdr-cron@v0.2.0
		...
		{
			// The module system already encodes it; a second marker would
			// produce "0.1.1+dirty-dirty".
...
// TestPrintVersionNeverShowsAnEmptyField guards the one-line form the issue
// template asks reporters to paste. A zero BuildInfo — which is what a caller
// with no linker stamp and no module context has — must still produce a
// readable line rather than "herdr-cron  (, )".
func TestPrintVersionNeverShowsAnEmptyField(t *testing.T) {
	t.Parallel()

	var buf bytes.Buffer
	if err := printVersion(&buf, &globals{}); err != nil {
		t.Fatalf("printVersion: %v", err)
	}
	got := buf.String()
	if !strings.HasPrefix(got, "herdr-cron dev (none, unknown) go") {
		t.Errorf("printVersion() = %q, want the placeholders spelled out", got)
	}
...
// TestVersionFlagReportsTheStamp is the end-to-end half: the flag has to reach
// the resolved BuildInfo, not a package global.
func TestVersionFlagReportsTheStamp(t *testing.T) {
	t.Parallel()

	root := NewRootCommand(BuildInfo{Version: "1.2.3", Commit: "abc1234", Date: "2026-09-02T00:00:00Z"})
	var buf bytes.Buffer
	root.SetOut(&buf)
	root.SetErr(&buf)
	...
	if got := buf.String(); !strings.HasPrefix(got, "herdr-cron 1.2.3 (abc1234, 2026-09-02T00:00:00Z) ") {
