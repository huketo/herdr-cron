...
.PHONY: build
build: ## Build the herdr-cron binary into bin/
	go build $(GOFLAGS) -trimpath -ldflags '$(LDFLAGS)' -o $(BIN) ./cmd/herdr-cron

.PHONY: install
install: ## Install herdr-cron into GOBIN
	go install $(GOFLAGS) -trimpath -ldflags '$(LDFLAGS)' ./cmd/herdr-cron
